...
# We will use LIME algorithym. LIME stands for Local Interpretable Model-Agnostic Explanations. Local interpretations help us understand model predictions for a single row of data or a group of similar rows.
...
# A code of 1 means it is doubted such violence is terrorism, while the zero code indicates an attack is terrorist. Therefore, we select only the rows with a zero value.
...
setnames(GTDDTS, old = c("iyear", "imonth", "iday", "extended", "country_txt",  "region_txt", "provstate", "city", "success", "multiple", "suicide", "attacktype1_txt", "gname", "targtype1_txt", "weaptype1_txt", "nkill", "nwound"), new = c("Year", "Month", "Day", "Extended", "Country",  "Region", "Province", "City", "Success", "Multiple", "Suicide", "Attack", "Group", "Target", "Weapon", "Dead", "Wounded"))
...
# We see that Iraq, Pakistan, India, Afghanistan and Colombia comprise the five countries with the highest terrorist attack counts. Chile, which rarely features in news about terrorism is listed as the 15th country with most attacks, so let's select that. Additionally, it is still numerous enough to provide a good sample.
...
# Here we create a new Lethal terrorist attack column. This variable is populated by zero and one values. If values in the old Dead column are zero, then they remain zero in the Lethal column, For all values other than zero in the Dead column, these have been transformed into a value of one in the Lethal attack column.
...
# We remove the year, country and weapon columns. This is required, as we are not interested in annual analysis, we don't need a country column as we are only dealing with Chile data and weapon is correlated with the attack variable. We also don't need the counts of dead in the Dead variable, as we are now interested in predicting fatality yes or no. 
...
# Factor Variables have been amended, whereby all variable levels with a count of less than 5% of the total count of the variable have been removed. This is done to enhance data visualisation of the plots and interpretability of the results.
...
# This section provides code which documents various statistical summaries of the data. This allows the user to understand the various dimensions and characteristics of the data.
...
# It is important to use stratified sampling. This technique consists of forcing the distribution of the target variable(s) among the different splits to be the same. Otherwise, there is an imbalance in the data, which may interfere with how the algorithm generates predictions, as it will favour the class of the target variable with the highest count. Stratified sampling can be carried out with the createDataPartition function in the caret package. We create a slit of 75% for training data and 25% for test data. We specify we don't want a list using the list = False() parameter
...
# The below plot indicates that police target, assassination attack and bomb attack are the three most important variables in terms of predicting Lethal terrorist attack
...
# The text label '1' shows what value of target variable is being explained, which in this case, we are interested in explaining the predictions where a terrorist attack is not lethal. The probability shows the probability of the observation belong to the label no. Inside each plot, there are several bar charts. The y-axis indicates the key features while the x-axis show the relative strength of each features. The positive value (blue color) indicates that the feature supports or increase the value of the prediction, while the negative value (red color) has a negative effect or decrease the prediction value.
# ALl the predicted probabilities of non lethal attack are high, which is just as well, as when we compare these predictions with the actual values in the test data, for these five rows, every row is not lethal attack:

# We can compare the results with the actual test data values:
  
head(Chile_test$Lethal)  

# Surprisingly, in the three rows where bomb attack is one of the five key variables, it signficantly supports the prediction of non lethal attack. Assassination attack strongly contradicts the prediction of non lethal attack. In all cases, Santiago city supports the prediction of lethal attack, as does all groups apart from the Movement of the Revolutionary Left. All target variables positively supported the prediction of lethal attack.
